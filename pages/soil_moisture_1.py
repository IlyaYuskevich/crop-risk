...
for yr in years:
    mapper = fsspec.get_mapper(
        f"s3://{BUCKET}/era5-land/volumetric_soil_water_1_{yr}.zarr", s3=storage
    )
    try:
        dss.append(xr.open_zarr(mapper, consolidated=True))
    except Exception as e:
        print(f"[soil moisture-1] missing or unreadable store for {yr}: {e}")
        continue

if not dss:
    st.warning("No ERA5-Land soil moisture (Level 1) data found for the selected crop year(s).")
    st.stop()
elif len(dss) == 1:
    ds = dss[0]
else:
...
add_timeseries_chart(
    ...
    {
        "y_title": None,
        "chart_title": "Soil moisture (0 - 7 cm)",
        "hovertemplate": "Date: %{x|%Y-%m-%d}<br>swvl1: %{y:.2f}<extra></extra>",
        "line_legend_label": "Volumetric soil water layer 1",
    },
)
