...
from constants.heat import AGRI_MONTHS, STAGE_MARKERS
...
lf = pl.scan_parquet("./data/source.parquet").with_columns(
        (pl.col("temperature_2m_max") - 273.15).alias("temp_max_celcius")
    ).with_columns(
    ...
    pl.when(pl.col("dt").dt.month().is_in([9, 10, 11, 12]))
      ...
      .alias("agri_year"),
    pl.col("temp_max_celcius").rolling_mean_by("dt", window_size="3d").over("name").alias("temp_max_3d")
)
...
region_year = (
    lf.filter(
      ...
      .select(["dt", "temp_max_3d", "agri_year"])
      .sort("dt")
      .collect()
)
...
for s in STAGE_MARKERS:
    ...
    stage_rows.append({"label": s["label"], "stage_start": start, "warn_temp": s["warn_temp"], "alert_temp": s["alert_temp"], "low_temp": s["low_temp"]})
...
if region_year.height > 0:
    ...
    fig.update_layout(
        title=dict(
            text=f"{region} | Heat Stress <br><span style='font-weight:normal; font-size:0.8em;'>(Crop year {year_sel})</span>",
            x=0.5,
            xanchor="center"
        ),
            height=600,
